...
1. What is the purpose of using _sessions_?
    HTTP is stateless so if we signed in our app would lose who we are after every request. User posts credentials session created, session id is stored on s cookie in the browser
1. What does bcrypt do to help us store passwords in a secure manner.
Uses a slower hashing function to protect from brute force attacks 
1. What does bcrypt do to slow down attackers?
    Also salts the password by adding random data that protects against some other sophisticated attacks
